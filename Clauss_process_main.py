...
from Clauss_burner import clauss_burner
...
result = sc.odeint(lambda y, V: clauss_burner(y, V, pars), yini, Vs)
...
plt.figure("Burner Simulation", figsize = (8, 7.5))

plt.subplot(2, 1, 1)
plt.plot(Vs, nH2S, Vs, nSO2, Vs, nS2, Vs, nH2O, Vs, nO2, Vs, nN2)
plt.ylim(0, 6)
plt.xlim(0, 1)
plt.title("Mole Changeover in Volume of Burner")
plt.xlabel("Volume [m3]")
plt.ylabel("Mole Flow [mole/s]")
plt.legend(["nH2S", "nSO2", "nS2", "nH2O", "nO2", "nN2"], loc = "center right", bbox_to_anchor=(0, 0.5, 1, 0.4), fontsize = "small", ncol = 2)
...
# scipy.integrate.ode(f).set_integrator('lsoda')
# figure(1)
# subplot(2,1,1)
